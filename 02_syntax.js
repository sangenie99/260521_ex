...
// 강제는 아닌데 모두의 약속 case
// camelCase -> (첫글자는 소문자, 두번째 글자부터 대문자)
// your name -> 공백은 쓸 수 없음 -> yourName 
// snake_case, PascalCase, kebab-case -> 각각의 프로그래밍 언어가 선호하는게 있음
// css -> kebab-case 선호
// 언어들 마다 선호하는 case가 있다.

// 키워드 - 미리 정해져 있는 단어
// 예약어라고도 한다. 예약어는 네임스페이스로 사용할 수 없다.
// let -> 변수를 선언하는 키워드
// for -> 반복 관련 기능을 처리하는 키워드


// 코드의 줄이 바뀐다 -> python 엔터만으로도 인식을 하지만 javascript는 인식 x
// 이 코드 줄이 끝났다 -> compiler/interpreter -> 코드해석기에게 전달
// javascript ';' -> 문장의 끝을 알리는 표시 but 생략가능한 경우가 많다. 
console.log("");
// 뒤에 뭐가 올거라고 예산되는 키워드나 ()[]{} 등은 세미콜론을 알아서 붙여주지 않음 (자동 줄바꿈x)
